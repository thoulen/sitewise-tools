...
def list_associated_assets(asset_id, hierarchy_id):
    ...
    while True:
        if next_token:
            response = sw_client.list_associated_assets(
                assetId=asset_id, hierarchyId=hierarchy_id, nextToken=next_token
            )
        else:
            response = sw_client.list_associated_assets(
                assetId=asset_id, hierarchyId=hierarchy_id
            )
    ...
    for father_hierarchy in father_hierarchy_list:
        ...
        for child_asset in associated_assets:
            ...
            if len(child_asset["hierarchies"]) > 0:
                ...
                delete_assets_hierarchy(
                    father_asset_id=child_asset_id,
                    father_hierarchy_list=child_asset["hierarchies"],
                )

            # Disassociate
            print(
                f"\nDisassociate Child Asset Name: {child_asset_name}, \
                    Asset Id: {child_asset_id}, Father Id: {father_asset_id}, \
                    Hierarchy Id: {father_hierarchy_id}"
            )
            sw_client.disassociate_assets(
                assetId=father_asset_id,
            ...
            while retry <= MAX_RETRY and retry >= 0:
                ...
                try:
                    print(
                        f"\nDelete Child Asset Name: {child_asset_name}, \
                            Asset Id: {child_asset_id}, try:{retry}"
                    )
...
def delete_model(model_ids):
    for model_id in model_ids:
        print(model_id)
        try:
            model = sw_client.describe_asset_model(
                assetModelId=model_id["childAssetModelId"]
            )
            ...
            if model["assetModelStatus"]["state"] == "ACTIVE":
                ...
                print(
                    f"\nDelete Asset Model Name: {model['assetModelName']}, \
                        Asset Model Id: {model['assetModelId']}, \
                        Status : {model['assetModelStatus']['state'] }"
                )
                sw_client.delete_asset_model(assetModelId=model["assetModelId"])
                print("Succeed")
            else:
                print(
                    f"\nSkipped Asset Model Name: {model['assetModelName']}, \
                        Asset Model Id: {model['assetModelId']}, \
                        Status : {model['assetModelStatus']['state'] }"
                )
    ...
    for model in model_list:
        try:
            if model["status"]["state"] == "ACTIVE":
                ...
                print(
                    f"\nSkipped Asset Model Name: {model['name']}, \
...
def get_asset_model(model_id):
    asset_model = sw_client.describe_asset_model(
        assetModelId=model_id, excludeProperties=True
    )
...
if __name__ == "__main__":
    ...
    # Add the arguments
    parser.add_argument("-o","--object", help="Object ['model','asset']")
    parser.add_argument("-p","--operation", help="Operation ['tree','view','drop','wipe']")
    parser.add_argument("-u", "--uid",type=sitewise_uuid,default=None,
        help="ID of the model to delete otherwise all models will be deleted ",
    )
    parser.add_argument("-a",
        "--all-levels", help="Include all levels in the hierarchy", action="store_true"
    )
    ...
    print(f"Entity: {sitewise_entity}, Operation: {operation}, UID:{args.uid}" )
    if (sitewise_entity == "asset") and ( operation in ("wipe","drop")    ):
        asset_drop(args.uid)
        sys.exit()

    if (sitewise_entity == "model") and (operation in ("wipe","drop")    ):
        asset_model_drop(args.uid)
        sys.exit()
